...
spm.load("/kaggle/input/spm-model-finale/pytorch/default/1/spm_final/nepali_spm_final.model")
...
class Dataloader:
    def __init__(self,B,T):
        ...
        "/kaggle/input/nepali-courpus/Nepali_tokens_v3.bin", dtype=np.uint16, mode="r"
       )
    ...
    def get_state(self):
        ...
        return {
            'train_position': self.trainPosition,
            'val_position': self.valPosition
        }
    
    def load_state(self, state):
        """Restore state from checkpoint"""
        self.trainPosition = state.get('train_position', 0)
        self.valPosition = state.get('val_position', 0)
        print(f"Dataloader state restored: train={self.trainPosition}, val={self.valPosition}")
...
def estimate_loss(B):
    ...
    saved_train_pos = dataloader.trainPosition
    saved_val_pos = dataloader.valPosition
    ...
    # Restore positions - THIS IS CRITICAL!
    dataloader.trainPosition = saved_train_pos
    dataloader.valPosition = saved_val_pos
...
min_lr = 4e-5  # Min LR
warmup_iters = 400       # How many steps to warm up
lr_decay_iters = 20000  # Total steps (should match your epochs)
...
optimizer = configure_optimizers(
    model, weight_decay=1e-1, learning_rate=learning_rate, device_type=device
)
...
def save_checkpoint(step, model: TinyLM,dataloader, optimizer, loss, filename="checkpoint.pth"):
...
def load_checkpoint(model: TinyLM,dataloader, optimizer, filename="checkpoint.pth"):
    # file_path=os.path.join(checkout_output,filename)
    if os.path.exists(filename):
        ...
        if "dataloader_state" in checkpoint:
            dataloader.load_state(checkpoint["dataloader_state"])
...
start_step, start_loss = load_checkpoint(
    ...
    filename="/kaggle/input/thebest-checkpoint/pytorch/default/1/The_best_128M.pth",
)
...
for step in range(start_step, epochs):
    ...
    if step % 100 == 0:
        ...
        print(f"Step {step}, LR: {current_lr:.2e}, train: {losses['train']:.4f}, val: {losses['val']:.4f}")  # UPDATE

        with open("/kaggle/working/losses.csv", "a", encoding="utf-8") as f:
            f.write(
                f"Step {step}, LR: {current_lr:.2e}, train :{losses['train']:.4f}  Val: {losses['val']:.4f}\n"
            )
            f.flush()
